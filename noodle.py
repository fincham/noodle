...
class Sample:
    def __init__(
        ...
        if path is None:
            raise NoodleImpossibleSampleDefinitionError(
                "You must specify a path to the sample."
            )
    ...
    def play(self):
        ...
        log(
            "player",
            (Colors.GREEN if not self.one_shot else Colors.BLUE)
            + "P"
            + Colors.END
            + " %s" % str(self),
        )
    ...
    def stop(self):
        ...
        log(
            "player",
            Colors.YELLOW
            + ("S" if self.fade_out is None else "F")
            + Colors.END
            + " %s" % str(self),
        )
...
class Instrument:
    def __init__(
        self, name=None, device_number=None, zero_velocity_for_note_off=False, uid=None
    ):
        ...
        for device_number in range(pypm.CountDevices()):
            interface, name, input_device, output_device, opened = pypm.GetDeviceInfo(
                device_number
            )
            if input_device == 1:
    ...
    def midi_status_decode(self, status_code, data_two):
        for candidate_status_code in MIDI_MESSAGE_STATUSES:
            if (
                ...
                and status_code <= candidate_status_code + MIDI_CHANNELS - 1
            ):
                status = MIDI_MESSAGE_STATUSES[candidate_status_code]
                if (
                    self.zero_velocity_for_note_off
                    and status == "note_on"
                    and data_two == 0
                ):  # weird dumb MIDI devices like the MPC2000XL do this
                    status = "note_off"
                return (
                    status,
                    status_code - candidate_status_code,
                )  # return a tuple of the status and the channel
...
def main():
    ...
    for device_number in range(pypm.CountDevices()):
        ...
        log(
            ...
            % (
                device_number,
                name,
                "(input)" if input_device == 1 else "(not input, ignored)",
            ),
        )
    ...
    for instrument in config["instruments"]:
        try:
            instruments.append(
                Instrument(
                    uid=instrument.get("uid", None),
                    name=instrument.get("name", None),
                    device_number=instrument.get("device_number", None),
                    zero_velocity_for_note_off=instrument.get(
                        "zero_velocity_for_note_off", False
                    ),
                )
            )
    ...
    for sample_definition in config["samples"]:
        ...
        samples[sample_definition["note"]].append(
            Sample(
                sample_definition.get("path", None),
                one_shot=sample_definition.get("one_shot", False),
                fade_out=sample_definition.get("fade_out", None),
                instrument=sample_definition.get("instrument", None),
                pan=sample_definition.get("pan", 127),
                no_overlap=sample_definition.get("no_overlap", False),
                debounce=sample_definition.get("debounce", 0),
            )
        )
    ...
    while True:
        for instrument in instruments:
            while instrument.midi_device.Poll():
                ...
                try:
                    status, channel = instrument.midi_status_decode(
                        status_code, data_two
                    )
                ...
                log(
                    "midi",
                    "%s, channel %i, data_one %i, data_two %i"
                    % (status, channel, data_one, data_two),
                )
                ...
                try:
                    if status != "control_change":
                        ...
                        for sample in this_note_samples:
                            if (
                                sample.instrument is None
                                or instrument.uid == sample.instrument
                            ):
                                if status == "note_on":
                                    sample.play()
                    ...
                    log(
                        ...
                        + "E"
                        + Colors.END
                        + " No sample for note %i on channel %i" % (e[0], channel),
                    )
